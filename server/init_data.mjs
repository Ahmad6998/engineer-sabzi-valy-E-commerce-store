...
import { TODAY_MANDI_RATES } from '../src/data/mandiRates.js';
import { STORE_SETTINGS } from '../src/data/areas.js';
...
fs.writeFileSync(path.join(DATA_DIR, 'orders.json'), JSON.stringify(INITIAL_ORDERS, null, 2), 'utf-8');
fs.writeFileSync(path.join(DATA_DIR, 'mandiRates.json'), JSON.stringify(TODAY_MANDI_RATES, null, 2), 'utf-8');
fs.writeFileSync(path.join(DATA_DIR, 'settings.json'), JSON.stringify(STORE_SETTINGS, null, 2), 'utf-8');

// Also sync products.json to public and src/data for static builds
fs.writeFileSync(path.resolve(ROOT_DIR, 'public/store_products.json'), JSON.stringify(PRODUCTS, null, 2), 'utf-8');
fs.writeFileSync(path.resolve(ROOT_DIR, 'src/data/store_products.json'), JSON.stringify(PRODUCTS, null, 2), 'utf-8');

console.log('✅ All backend database files initialized successfully in server/data/');
